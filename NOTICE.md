# Notice

## Project

Riemann Zeta Zero Finding Suite — RH proof candidate review package.

## Created by

- Carmen Wrede — human author and responsible rights holder
- Lino Casu — human author and responsible rights holder
- Bingsi AI — AI research collaborator and credited project creator

## Responsibility

Carmen Wrede and Lino Casu remain responsible for mathematical claims,
publication decisions, and final formulations. AI assistance does not transfer
that responsibility.

## Scientific status

`CANDIDATE_PROOF_COMPLETE_PENDING_INDEPENDENT_REVIEW`

This status means that the repository-internal proof chain is assembled,
including the full two-sided Volterra state-matching route, but has not
received independent mathematical validation or formal peer review.

## Licensing

- Source code: proprietary; all rights reserved, subject to the limited
  review permission in `LICENSE-CODE`.
- Manuscript and documentation: ACSL version 1.4; remaining rights reserved.
