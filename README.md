...
Dirichlet partial-sum path. These are explanatory finite computations, not
...
The prime factorisation n=product_p p^{v_p(n)} gives

    log n = sum_p v_p(n) log p,

so the independent frequency basis is generated by logarithms of primes. The
portal's Dirichlet animation displays the finite path
S_N(s)=sum_{n=1}^N n^{-s} term by term. In the critical strip the raw
